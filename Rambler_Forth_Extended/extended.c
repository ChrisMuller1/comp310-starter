...
#define VARIABLE_MAX_SIZE 100

typedef struct {
    int data[STACK_MAX_SIZE];
    int top;
    bool boolean_data[STACK_MAX_SIZE];
    char variables[VARIABLE_MAX_SIZE][STACK_MAX_SIZE];
    int values[VARIABLE_MAX_SIZE];
    int var_count;
} IntStack;
...
void int_stack_and(IntStack *stack) {
    bool a, b;
    if (stack->top < 1) {
        printf("Not enough elements for AND operation\n");
        exit(EXIT_FAILURE);
    }
    a = stack->boolean_data[stack->top];
    b = stack->boolean_data[stack->top - 1];
    stack->top--;
    stack->boolean_data[stack->top] = a && b;
}

// Add functions to handle looping logic
void int_stack_while(IntStack *stack) {
    while (stack->boolean_data[stack->top]) {
        printf("Loop");
    }
}
void int_stack_push_variable(IntStack *stack, const char *name, int value) {
    if (stack->var_count >= VARIABLE_MAX_SIZE - 1) {
        printf("Variable limit reached\n");
        return;
    }
    strcpy(stack->variables[stack->var_count], name);
    stack->values[stack->var_count++] = value;
}

// Function to get the value of a variable by name
int int_stack_get_variable_value(const IntStack *stack, const char *name) {
    for (int i = 0; i < stack->var_count; ++i) {
        if (strcmp(stack->variables[i], name) == 0) {
...
void int_stack_display_variables(const IntStack *stack) {
    printf("Variables:\n");
    for (int i = 0; i < stack->var_count; ++i) {
        printf("%s = %d", stack->variables[i], stack->values[i]);
    }
}
int main() {
    ...
    IntStack stack1;
    int_stack_init(&stack1);
    int_stack_push(&stack1, 1);
    int_stack_push(&stack1, 0);
    int_stack_and(&stack1);
    // false (0)
    printf("Result: %d\n", stack1.boolean_data[stack1.top]);

    // Testing loops
    int_stack_push(&stack1, 1);
    int_stack_push(&stack1, 2); // Pushing a true value
    int_stack_push(&stack1, stack1.boolean_data[stack1.top]); 
    int_stack_while(&stack1);

    IntStack stack2;
    int_stack_init(&stack2);

    // Push variables on to stack
    int_stack_push_variable(&stack2, "chris", 27);
    int_stack_push(&stack2, int_stack_get_variable_value(&stack2, "chris"));
    int_stack_push(&stack2, 2);
    int_stack_push(&stack2, 3);
    // Display variables and stack
    int_stack_display_variables(&stack2);
    int_stack_display(&stack2);
